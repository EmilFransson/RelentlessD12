...
#include "UI/Widgets/IWidgetContainer.h"
...
namespace Relentless
{
	...
	protected:
		NO_DISCARD virtual Ref<ITableRow> GenerateNewWidget(const ItemType& item) noexcept;
	...
	uint32 ListView<ItemType>::GetItemIndex(const ItemType& item) const noexcept
	{
		...
		RLS_ASSERT(false, "[ListView::GetItemIndex]: Undefined behavior - item not found.");
	...
	void ListView<ItemType>::OnRender() noexcept
	{
		...
		if (ImGui::BeginTable("##Table", numColumns, this->GetFlags()))
		{
			...
			while (clipper.Step())
			{
				...
				if (!warmUp)
				{
					if (static_cast<int>(m_VisibleListStart) != clipper.DisplayStart || static_cast<int>(m_VisibleListEnd) != clipper.DisplayEnd)
					{
						m_VisibleListStart = static_cast<uint32>(clipper.DisplayStart);
						m_VisibleListEnd = static_cast<uint32>(clipper.DisplayEnd);
						clipRangeDirty = true;
					}
				}
