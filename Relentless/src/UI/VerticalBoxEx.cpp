#include "VerticalBoxEx.h"

namespace Relentless
{
	VerticalBoxEx::VerticalBoxEx(const Vector2 aSize, bool aIsChildRegion) noexcept
		: IWidgetContainer<VerticalBoxEx>(aSize, aIsChildRegion)
	{
	}

	Vector2 VerticalBoxEx::ReportSize() const noexcept
	{
	...
	VerticalBoxEx* VerticalBoxEx::SetSpacing(float aSpacing) noexcept
	{
		m_Spacing = Vector2(0.0f, aSpacing);
		return this;
	}

	void VerticalBoxEx::OnRender() noexcept
	{
		if (!IsVisible())
			return;
