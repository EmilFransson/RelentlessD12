...
namespace Relentless
{
	...
	public:
		...
		virtual NO_DISCARD bool IsContainer() const noexcept override 
		{ 
			return false; 
		};
	...
	void TileView<ItemType>::OnRender() noexcept
	{
		...
		const float availableHeight = availableSpace.y;
		...
		while (clipper.Step())
		{
			...
			if (this->m_VisibleListStart != clipper.DisplayStart * numItemsPerRow || this->m_VisibleListEnd != Math::Min((clipper.DisplayEnd * numItemsPerRow), numItems))
			{
