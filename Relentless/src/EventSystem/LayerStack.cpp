...
namespace Relentless
{
	...
	void LayerStack::PopLayer(Layer* layer) noexcept
	{
		...
		RLS_ASSERT(it < m_layers.begin() + m_nrOfRegularLayers, "Layer does not exist in stack.");
		layer->OnDetach();
		m_layers.erase(it);
		m_nrOfRegularLayers--;
	}
	...
	{
		auto it = std::find(m_layers.begin(), m_layers.end(), layer);
		RLS_ASSERT(it >= m_layers.begin() + m_nrOfRegularLayers, "Overlay does not exist in stack.");
		layer->OnDetach();
		m_layers.erase(it);
	}
