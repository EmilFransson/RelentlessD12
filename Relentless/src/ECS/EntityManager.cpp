...
{
	constexpr const uint32_t INITIAL_SYSTEM_CAPACITY = 250u;
	...
	{
		RLS_ASSERT(!m_freeList.empty(), "Entity capacity reached!");
		uint32_t indexToInsert = m_freeList.front();
	...
	void EntityManager::Initialize() noexcept
	{
		...
		for (uint32_t entityId{ 0u }; entityId < MAX_ENTITIES; entityId++)
			m_freeList.push(entityId);
	...
	{
		ECS_DEBUG_OP([&]() { for (auto& system : m_systems) RLS_ASSERT(typeid(*system) != typeid(*pSystem), "System already exists."); })
		pSystem->Create();
