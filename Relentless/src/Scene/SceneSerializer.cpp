...
namespace Relentless
{
	static void SerializeEntity(YAML::Emitter& out, EntityManager& manager, entity entity) noexcept
	{
		...
		if (manager.Has<MeshRendererComponent>(entity))
		{
			...
			MaterialSerializer::Serialize(manager.Get<MeshRendererComponent>(entity).MaterialHandle, AssetManager::GetAssetPath(manager.Get<MeshRendererComponent>(entity).MaterialHandle.UUID));
	...
	bool SceneSerializer::Deserialize(const std::shared_ptr<Scene>& pScene, const std::string& filepath) noexcept
	{
		...
		for (auto entity : entities)
		{
			...
			if (meshFilterComponent)
			{
				...
				std::string meshPath = AssetManager::GetAssetPath(uuid);
				mfc.MeshHandle = AssetManager::Load<Mesh>(meshPath);
			}
			...
			if (meshRendererComponent)
			{
				...
				std::string materialPath = AssetManager::GetAssetPath(uuid);
				mrc.MaterialHandle = AssetManager::Load<Material>(materialPath);
	...
	void Serialize(const SceneHandle& sceneHandle, const std::string& filepath) noexcept
	{
		//RLS_ASSERT(sceneHandle != NULL_HANDLE, "Scene handle is invalid");
		//Scene& scene = AssetManager::Get<Scene>(sceneHandle);
		//
		//YAML::Emitter out;
		//out << YAML::BeginMap;
		//
		...
		//
		//out << YAML::Key << "Entities" << YAML::BeginSeq;
		//scene.GetEntityManager().Collect<TransformComponent>().Do([&](entity e)
		//	{
		//		SerializeEntity(out, scene.GetEntityManager(), e);
		//	});
		//
		//out << YAML::EndSeq;
		//out << YAML::EndMap;
	...
	SceneHandle Deserialize(const std::string& filepath) noexcept
	{
		RLS_ASSERT(std::filesystem::exists(filepath), "File path is invalid.");
		...
		//auto& sceneManager = AssetManager::GetSceneManager();
		//SceneHandle sceneHandle = sceneManager.CreateNewScene(filepath);
		...
		for (auto entity : entities)
		{
			RLS::entity deserializedEntity;
			{
				...
				if (entity["DirectionalLightComponent"])
				{
					deserializedEntity = scene.CreateLight(name.c_str(), LightType::Directional);
					mgr.Get<IDComponent>(deserializedEntity).UuId = uuid;
				}
				else if (entity["PointLightComponent"])
				{
					deserializedEntity = scene.CreateLight(name.c_str(), LightType::Point);
					mgr.Get<IDComponent>(deserializedEntity).UuId = uuid;
				}
				else
				{
					//Create "default" entity:
					deserializedEntity = scene.CreateEntityWithUUID(name.c_str(), uuid);
				}
			}
			...
			if (meshRendererComponent)
			{
				...
				std::string materialPath = AssetManager::GetAssetPath(uuid);
				mrc.MaterialHandle = AssetManager::Load<Material>(materialPath);
				scene.GetEntityManager().AddOrReplace<DirtyMeshRendererComponent>(deserializedEntity);
			}
