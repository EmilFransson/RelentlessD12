...
{
	enum class DescriptorHandleType { RTV = 0u, DSV, SRV, CBV, UAV, SRV_NV, CBV_NV, UAV_NV, None }; //NV = Non shader visible

	struct DescriptorHandle
	{
		D3D12_CPU_DESCRIPTOR_HANDLE CPUHandle;
		D3D12_GPU_DESCRIPTOR_HANDLE GPUHandle;
		DescriptorHandleType Type = DescriptorHandleType::None; //Set by descriptor manager!
		uint32_t Index;
#if defined(RLS_DEBUG)
		friend class DescriptorHeap;
		DescriptorHeap* pDebugInterface = nullptr;
#endif
	};
	...
	public:
		...
		DescriptorHeap(GraphicsDevice* pParent, const D3D12_DESCRIPTOR_HEAP_TYPE descriptorHeapType, const uint32_t capacity, const bool isShaderVisible) noexcept;
		virtual ~DescriptorHeap() noexcept override = default;
		[[nodiscard]] DescriptorHandle AllocateDescriptor() noexcept;
		[[nodiscard]] std::vector<DescriptorHandle> AllocateDescriptorBlock(uint32 blockSize) noexcept;
		void FreeDescriptor(const DescriptorHandle& descriptorHandle, const SyncPoint& syncPoint) noexcept;
	...
	private:
		...
		{
			FencedDescriptorHandle(const DescriptorHandle& descriptorHandle, const SyncPoint& syncPoint)
