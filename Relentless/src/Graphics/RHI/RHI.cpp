...
namespace Relentless
{
	...
	static constexpr FormatInfo sFormatInfo[] = {
		...
		{ FORMAT_TYPE(R16_TYPELESS),		FormatType::Typeless,	  2,		1,		1,			false, false, false, false }
	};
	...
	namespace RHI
	{
		const FormatInfo& GetFormatInfo(ResourceFormat format) noexcept
		{
			const FormatInfo& formatInfo = sFormatInfo[(uint8)format];
			RLS_ASSERT(formatInfo.Format == format, "Format mismatch error.");
			return formatInfo;
		}
		...
		uint64 GetRowPitch(ResourceFormat format, uint32 width, uint32 mipIndex) noexcept
		{
			const FormatInfo& info = GetFormatInfo(format);
			if (info.BlockSize > 0)
			{
		...
		uint64 GetSlicePitch(ResourceFormat format, uint32 width, uint32 height, uint32 mipIndex) noexcept
		{
			const FormatInfo& info = GetFormatInfo(format);
			if (info.BlockSize > 0)
			{
		...
		uint64 GetTextureMipByteSize(ResourceFormat format, uint32 width, uint32 height, uint32 depth, uint32 mipIndex) noexcept
		{
			return GetSlicePitch(format, width, height, mipIndex) * Math::Max(1u, depth >> mipIndex);
		}

		uint64 GetTextureByteSize(ResourceFormat format, uint32 width, uint32 height, uint32 depth, uint32 numMips) noexcept
		{
			uint64 size = 0;
